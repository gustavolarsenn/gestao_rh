...
import { CreateTeamKpiDto } from './dto/create-team-kpi.dto';
import { UpdateTeamKpiDto } from './dto/update-team-kpi.dto';
...
export class TeamKpisService {
  ...
  async create(dto: CreateTeamKpiDto): Promise<TeamKPI> {
    ...
    if (exists) throw new ConflictException('A TeamKPI for this period and source already exists.');
  ...
  async update(companyId: string, id: string, dto: UpdateTeamKpiDto): Promise<TeamKPI> {
    ...
    const keysChange =
      ...
      (dto.source && dto.source !== row.source);
